...
fn read_version_byte(transaction_bytes: &[u8]) -> Result<u32, Errors> {
    let mut buffer = [0; 4];
    transaction_bytes.read_exact(&mut buffer); // read exactly 4 bytes from transaction_bytes into the buffer stream;
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    ...
    match read_version_byte(&transaction_bytes) {
        Ok(version) => version,
        Err(Errors::IoError (e)) => Err(format!("I/O error reading version: {}", e)),
        Err(Error::TooShortError) => Err("Transaction data too short".to_string()),
        Err(Error::HexError(_)) => Err("Unexpected hex error during version read".to_string()),
    }
    // todo!()
}
